...
2. 给出恰好 5 个可编辑的选题方向，并结合已发布主题保持内容故事线连续性；本地记录缺失时可从创作后台只读同步。
3. 用一个内置的 Lingzao Skill 拆解热点笔记中可借鉴的标题、信息节奏与表达机制，不复制原文或原图。
4. 生成初稿，再通过内置中文去 AI 味 Skill 形成可编辑终稿。
5. 从本地上传用户自己的头像母版，由 Agent 提取人物、发型、穿着、比例与绘制方式的身份锁，并先生成 6 个可逐张预览的系列品牌形象；后续只改变动作、手势和表情。
6. 由选题和拆解推荐动态视觉方向，并由用户选择本轮生成 1–6 张配图；卡片文稿、角色动作、预览与发布数量始终一致。
7. 在完整预览中查看文稿和全部配图；可输入修改意见后再生成，或确认后选择“立即发布”或“暂缓发布”。
...
| 故事线 | Codex Agent + 当前账号只读会话 | 自动发布需 ID/URL 才入档；可从创作后台同步，后台登录失效时回退到已核验的本人主页 |
...
| 任务 | 推理强度 |
| --- | --- |
| 热点检索、头像/配图、纯视觉修改、故事线同步、发布 | `medium` |
...
- 故事线同步优先读取当前账号创作后台；后台登录失效时只允许回退到能证明为当前账号本人的公开主页。不编辑作品，不读取评论，也不会把失败、草稿或结果不明的任务升级为已发布。
